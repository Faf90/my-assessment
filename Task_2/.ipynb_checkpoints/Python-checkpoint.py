...
def question_1(df_balances):
    ...
    # Mark missed payments if the actual payment is less than the scheduled payment
    missed = df_balances["ActualRepayment"] < df_balances["ScheduledRepayment"]

    # Group By LoanID to calculate the percentage of loans which are Type 1
    defaulted_per_loan = missed.groupby(df_balances["LoanID"]).any()

    default_rate_percent = defaulted_per_loan.mean() * 100
...
def question_2(df_scheduled, df_balances):
    ...
    # Get the expected repayment per loan per year
    expected_total_repayment = df_scheduled.groupby("LoanID")["ScheduledRepayment"].first() * 12
    # Get the actual repayment per loan per year
    actual_total_repayment = df_balances.groupby("LoanID")["ActualRepayment"].sum() 

    # Get the unpaid ratio
    unpaid_ratio = (expected_total_repayment - actual_total_repayment) / expected_total_repayment

    # Determine the percentage loans where the unpaid ratio is more than 0.15
    default_rate_percent = (unpaid_ratio > 0.15).mean() * 100
...
def question_3(df_balances):
    ...
    # Unscheduled Principal: Extra principal paid beyond the scheduled principal
    # Unscheduled Principal = Actual Principal - Scheduled Principal 
    df["UnscheduledPrincipal"] = df["ActualRepayment"] - df["ScheduledRepayment"]

    # Clip negative principal to 0.0
    df["UnscheduledPrincipal"] = df["UnscheduledPrincipal"].clip(lower=0.0)

    # Group By month to get the total UnscheduledPrincipal and LoanBalanceStart per month
    monthly = df.groupby("Month")[["UnscheduledPrincipal", "LoanBalanceStart"]].sum()
    smm = monthly["UnscheduledPrincipal"] / monthly["LoanBalanceStart"]

    # Geometric mean SMM
    ssm_mean = np.prod(1 + smm) ** (1 / 12) - 1

    # Annualized Portfolio CPR
    cpr = 1 - ( 1 - ssm_mean) ** 12
    cpr_percent = cpr * 100
...
def question_4(df_balances):
    ...
    """ 
    Use Type 2 as it flags loans where more than 15% of the year's payments went unpaid.
    Type 1 flags a loan for missing a single payment by any amount. 
    Type 2 is more indicative of a loan that wont be paid back. 
    """
    probability_of_default = question_2(df_scheduled, df_balances) / 100

    # Get the outstanding balance at the start of year 2 which is at the end of month 12
    total_loan_balance = df_balances.loc[df_balances["Month"] == 12, "LoanBalanceEnd"].sum()
