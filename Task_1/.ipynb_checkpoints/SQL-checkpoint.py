...
def question_1():
    ...
    qry = """SELECT c.Name, c.Surname, c.CustomerId FROM customers c GROUP BY c.Name, c.Surname, c.CustomerId HAVING COUNT(c.CustomerId) > 1"""
...
def question_2():
    ...
    qry = """SELECT c.Name, c.Surname, c.Income FROM customers c WHERE c.Gender = 'Female' ORDER BY c.Income DESC"""
...
def question_3():
    ...
    qry = """WITH deduplicated AS (
                SELECT DISTINCT * FROM loans
            )
            SELECT LoanTerm, 
                100 * SUM (CASE WHEN ApprovalStatus = 'Approved' THEN 1 ELSE 0 END) / COUNT(*) AS ApprovedPercentage
            FROM deduplicated
            GROUP BY LoanTerm"""
...
def question_4():
    ...
    qry = """SELECT CustomerClass, COUNT(DISTINCT CustomerId) AS Count FROM credit
